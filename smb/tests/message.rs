...
fn test_as_bytes_consistency() {
    ...
    assert_eq!(
        bytes1, bytes2,
        "Byte representations should be identical for the same message content"
    );
    assert!(
        !bytes1.is_empty(),
        "Byte representation should not be empty"
    );
}
...
fn test_as_bytes_serialization_deserialization() {
    ...
    let (_, deserialized_message) =
        SMBMessage::<SMBSyncHeader, SMBBody>::parse(&serialized[4..]).unwrap();
