...
use crate::protocol::body::file_info::{
    ...
    FileAccessInformation, FileAccessFlags, FilePositionInformation,
    FileModeInformation, FileModeFlags,
    FileAlignmentInformation, FileAlignmentRequirement, FileNameInformation,
};
...
impl<S: Server> SMBLockedMessageHandlerBase for Arc<SMBTreeConnect<S>> {
    ...
    async fn handle_query_info(&mut self, header: &SMBSyncHeader, message: &SMBQueryInfoRequest) -> SMBResult<SMBHandlerState<Self::Inner>> {
        ...
        if max_output > 0 && data.len() > max_output {
            debug!(data_len = data.len(), max_output, "truncating response to output_buffer_length");
            data.truncate(max_output);
            let response = SMBQueryInfoResponse::new(data);
            let header = header.create_response_header(NTStatus::BufferOverflow as u32, header.session_id, header.tree_id);
            return Ok(SMBHandlerState::Finished(SMBMessage::new(header, SMBBody::QueryInfoResponse(response))));
        }
