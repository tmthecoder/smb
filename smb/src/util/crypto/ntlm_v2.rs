...
pub fn authenticate_v2(domain: &str, account: &str, password: &str, server_challenge: &[u8], lm_response: &[u8], nt_response: &[u8]) -> Result<(bool, Vec<u8>), anyhow::Error> {
    ...
    if resp {
        let response_key_nt = ntowf_v2(password, account, domain).unwrap();
        let nt_proof_str = &nt_response[0..16];
        let res = <Hmac<Md5> as Mac>::new_from_slice(&response_key_nt).unwrap()
...
fn ntowf_v2(password: &str, user: &str, domain: &str) -> Result<Vec<u8>, anyhow::Error> {
    ...
    let mut hmac_md5 = <Hmac<Md5>>::new_from_slice(password_hash.as_slice())?;
