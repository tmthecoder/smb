...
pub fn authenticate_v2(
    ...
    lm_response: &[u8],
    nt_response: &[u8],
) -> SMBResult<(bool, Vec<u8>)> {
    // AV-pairs structure
    let server_name = &nt_response[44..(nt_response.len() - 4)];
    let (nt_exp, lm_exp, _nt_proof) = compute_ntlm_v2_response(
        server_challenge,
        &nt_response[16..],
        server_name,
        password,
    ...
    let resp = if !resp {
        ...
        let expected_resp = compute_lmv2_response(
            server_challenge,
            lm_client_challenge,
            password,
            account,
    ...
    let resp = if !resp && nt_response.len() >= 16 {
        ...
        let expected_nt_proof = compute_ntlmv2_proof(
            server_challenge,
            client_structure_padded,
            password,
            account,
            domain,
        )?;
...
fn compute_ntlm_v2_response(
    ...
    account: &str,
    domain: &str,
) -> SMBResult<(Vec<u8>, Vec<u8>, Vec<u8>)> {
    ...
    let proof_hmac = new_hmac_from_slice(&key)?
        ...
        .chain_update(&temp);
    let nt_proof_str = hmac::Mac::finalize(proof_hmac).into_bytes();
    ...
    let lm_challenge_response_1 = hmac::Mac::finalize(lm_challenge_hmac).into_bytes();
...
fn compute_ntlmv2_proof(
    server_challenge: &[u8],
    client_structure_padded: &[u8],
    password: &str,
    account: &str,
    domain: &str,
) -> SMBResult<Vec<u8>> {
    ...
    let bytes_hmac = new_hmac_from_slice(&key)?;
    let bytes_hmac = bytes_hmac.chain_update(server_challenge).chain_update(temp);
    let result = hmac::Mac::finalize(bytes_hmac);
