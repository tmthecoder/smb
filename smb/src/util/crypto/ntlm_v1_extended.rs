...
pub fn authenticate_v1_extended(
    password: &str,
    server_challenge: &[u8],
    lm_response: &[u8],
    nt_respobse: &[u8],
    ...
    let expected_v1_response =
        compute_ntlmv1_extended_response(server_challenge, client_challenge, password)?;

    Ok(nt_respobse == expected_v1_response)
}

fn compute_ntlmv1_extended_response(
    server_challenge: &[u8],
    client_challenge: &[u8],
    password: &str,
) -> SMBResult<Vec<u8>> {
