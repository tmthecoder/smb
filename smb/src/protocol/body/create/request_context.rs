...
impl SMBFromBytes for CreateRequestContext {
        ...
        Self: Sized,
    {
        ...
        let context = match wrapper.name.as_slice() {
            ...
            QUERY_MAXIMAL_ACCESS_REQUEST_TAG if !wrapper.data.is_empty() => {
            ...
            QUERY_MAXIMAL_ACCESS_REQUEST_TAG if wrapper.data.is_empty() => {
                Ok(Self::QueryMaximalAccessRequest(QueryMaximalAccessRequest {
...
impl SMBToBytes for CreateRequestContext {
    fn smb_to_bytes(&self) -> Vec<u8> {
        match self {
            ...
            CreateRequestContext::DurableHandleRequest(x) => {
                create_ctx_smb_to_bytes!(x, DURABLE_HANDLE_REQUEST_TAG)
            }
            CreateRequestContext::DurableHandleReconnect(x) => {
                create_ctx_smb_to_bytes!(x, DURABLE_HANDLE_RECONNECT_TAG)
            }
            CreateRequestContext::AllocationSize(x) => {
                create_ctx_smb_to_bytes!(x, ALLOCATION_SIZE_TAG)
            }
            CreateRequestContext::QueryMaximalAccessRequest(x) => {
                create_ctx_smb_to_bytes!(x, QUERY_MAXIMAL_ACCESS_REQUEST_TAG)
            }
            CreateRequestContext::TimewarpToken(x) => {
                create_ctx_smb_to_bytes!(x, TIMEWARP_TOKEN_TAG)
            }
            CreateRequestContext::QueryOnDiskID(x) => {
                create_ctx_smb_to_bytes!(x, QUERY_ON_DISK_ID_TAG)
            }
            CreateRequestContext::RequestLease(x) => create_ctx_smb_to_bytes!(x, REQUEST_LEASE_TAG),
            CreateRequestContext::RequestLeaseV2(x) => {
                create_ctx_smb_to_bytes!(x, REQUEST_LEASE_TAG)
            }
            CreateRequestContext::DurableHandleRequestV2(x) => {
                create_ctx_smb_to_bytes!(x, DURABLE_HANDLE_REQUEST_V2_TAG)
            }
            CreateRequestContext::DurableHandleReconnectV2(x) => {
                create_ctx_smb_to_bytes!(x, DURABLE_HANDLE_RECONNECT_V2_TAG)
            }
            CreateRequestContext::AppInstanceID(x) => {
                create_ctx_smb_to_bytes!(x, APP_INSTANCE_ID_TAG)
            }
            CreateRequestContext::AppInstanceVersion(x) => {
                create_ctx_smb_to_bytes!(x, APP_INSTANCE_VERSION_TAG)
            }
            CreateRequestContext::SVHDXOpenDeviceContext(x) => {
...
pub struct EABuffer {
    ...
    #[smb_string(
        order = 0,
        length(inner(start = 5, num_type = "u8")),
        underlying = "u8"
    )]
    name: String,
