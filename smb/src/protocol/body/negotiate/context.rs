...
impl SMBFromBytes for NegotiateContext {
    ...
    {
        if input.len() < 4 {
            return Err(SMBError::parse_error("Input too small"));
        }
...
impl NegotiateContext {
    ...
    pub fn from_connection_state<R: SMBReadStream, W: SMBWriteStream, S: Server>(
        connection: &SMBConnection<R, W, S>,
        request_contexts: HashSet<u16>,
    ) -> Vec<Self> {
        ...
        if request_contexts.contains(&PRE_AUTH_INTEGRITY_CAPABILITIES_TAG) {
            response_contexts.push(Self::PreAuthIntegrityCapabilities(
                PreAuthIntegrityCapabilities::from_connection_state(connection),
            ));
        }
        if request_contexts.contains(&ENCRYPTION_CAPABILITIES_TAG) {
            response_contexts.push(Self::EncryptionCapabilities(
                EncryptionCapabilities::from_connection_state(connection),
            ));
        }
        if request_contexts.contains(&COMPRESSION_CAPABILITIES_TAG) {
            response_contexts.push(Self::CompressionCapabilities(
                CompressionCapabilities::from_connection_state(connection),
            ));
        }
        if request_contexts.contains(&RDMA_TRANSFORM_CAPABILITIES_TAG) {
            response_contexts.push(Self::RDMATransformCapabilities(
                RDMATransformCapabilities::from_connection_state(connection),
            ));
        }
        if request_contexts.contains(&SIGNING_CAPABILITIES_TAG) {
            response_contexts.push(Self::SigningCapabilities(
                SigningCapabilities::from_connection_state(connection),
            ));
        }
        if request_contexts.contains(&TRANSPORT_CAPABILITIES_TAG) {
            response_contexts.push(Self::TransportCapabilities(
                TransportCapabilities::from_connection_state(connection),
            ));
        }
        if request_contexts.contains(&POSIX_EXTENSIONS_TAG) {
            response_contexts.push(Self::PosixExtensions(
                PosixExtensions::from_connection_state(connection),
            ));
        }
        response_contexts
    }
    ...
    pub fn validate_and_set_state<R: SMBReadStream, W: SMBWriteStream, S: Server>(
        &self,
        connection: SMBConnectionUpdate<R, W, S>,
        server: &S,
    ) -> SMBResult<(SMBConnectionUpdate<R, W, S>, bool)> {
        match self {
            NegotiateContext::PreAuthIntegrityCapabilities(x) => {
                x.validate_and_set_state(connection)
            }
            NegotiateContext::EncryptionCapabilities(x) => x.validate_and_set_state(connection),
            NegotiateContext::CompressionCapabilities(x) => {
                x.validate_and_set_state(connection, server)
            }
            ...
            NegotiateContext::RDMATransformCapabilities(x) => {
                x.validate_and_set_state(connection, server)
            }
...
impl PreAuthIntegrityCapabilities {
    ...
    fn from_connection_state<R: SMBReadStream, W: SMBWriteStream, S: Server>(
        connection: &SMBConnection<R, W, S>,
    ) -> Self {
        ...
        if let Some(algorithm) = self.hash_algorithms.first() {
            Ok((
                connection
                    ...
                    .preauth_integrity_hash_value(Vec::new()),
                true,
            ))
        } else {
...
impl CompressionCapabilities {
    ...
    pub fn validate_and_set_state<R: SMBReadStream, W: SMBWriteStream, S: Server>(
        ...
        Ok((
            connection.compression_ids(self.compression_algorithms.clone()),
            true,
        ))
    }
}
...
pub struct NetnameNegotiateContextID {
    ...
    #[smb_string(
        order = 1,
        length(inner(start = 0, num_type = "u16")),
        underlying = "u16"
    )]
...
impl TransportCapabilities {
    ...
    pub fn validate_and_set_state<R: SMBReadStream, W: SMBWriteStream, S: Server>(
        ...
        if self
            .flags
            .contains(TransportCapabilitiesFlags::ACCEPT_TRANSPORT_LEVEL_SECURITY)
        {
...
impl RDMATransformCapabilities {
    ...
    pub fn validate_and_set_state<R: SMBReadStream, W: SMBWriteStream, S: Server>(
        ...
        Ok((
            connection.rdma_transform_ids(self.transform_ids.clone()),
            true,
        ))
    }
}
...
impl SigningCapabilities {
    ...
    pub fn validate_and_set_state<R: SMBReadStream, W: SMBWriteStream, S: Server>(
        ...
        algorithms.sort();
        Ok((
            connection.signing_algorithm_id(*algorithms.first().unwrap()),
            true,
        ))
    }
}
...
#[derive(
    Debug, Eq, PartialEq, Serialize, Deserialize, Clone, SMBFromBytes, SMBToBytes, SMBByteSize,
)]
