...
pub trait Body<S: Header>: SMBEnumFromBytes + SMBToBytes {
    fn parse_with_cc(bytes: &[u8], command_code: S::CommandCode) -> SMBParseResult<&[u8], Self>
    where
        Self: Sized;
...
#[derive(
    Serialize, Deserialize, PartialEq, Eq, Debug, SMBEnumFromBytes, SMBToBytes, SMBByteSize, Clone,
)]
pub enum SMBBody {
...
impl smb_core::SMBEnumFromBytes for LegacySMBBody {
    ...
    where
        Self: Sized,
    {
        match LegacySMBCommandCode::try_from(discriminator as u8)
            .map(|x| x == LegacySMBCommandCode::Negotiate)
        {
            Ok(true) => {
                let (remaining, cnt) =
                    le_u8(input).map_err(|_: nom::Err<nom::error::Error<&[u8]>>| {
                        SMBError::parse_error("Invalid count")
                    })?;
                let (_, protocol_vecs) = many1(take_till(|n: u8| n == 0x02))(remaining).map_err(
                    |_: nom::Err<nom::error::Error<&[u8]>>| {
                        SMBError::parse_error("No valid payload")
                    },
                )?;
                ...
                for slice in protocol_vecs {
                    ...
                    protocol_strs.push(
                        String::from_utf8(vec).map_err(|_| {
                            SMBError::parse_error("Could not map protocol to string")
                        })?,
                    );
                }
                let (remaining, _) = take(cnt as usize)(input).map_err(
                    |_: nom::Err<nom::error::Error<&[u8]>>| {
                        SMBError::parse_error("Size too small for parse length")
                    },
                )?;
                Ok((remaining, LegacySMBBody::Negotiate(protocol_strs)))
            }
            _ => Err(SMBError::parse_error(
                "Unknown parse error for LegacySMBBody",
            )),
        }
    }
}
...
impl Body<LegacySMBHeader> for LegacySMBBody {
    fn parse_with_cc(
        bytes: &[u8],
        command_code: LegacySMBCommandCode,
