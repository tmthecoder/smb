...
pub enum SMBTreeConnectBuffer {
    ...
    #[smb_string(
        order = 0,
        start(inner(start = 0, num_type = "u16", subtract = 68)),
        length(inner(start = 2, num_type = "u16")),
        underlying = "u16"
    )]
    Path(String),
...
pub struct SMBTreeConnectExtension {
    ...
    #[smb_string(
        order = 1,
        start(inner(start = 2, num_type = "u16", subtract = 64)),
    ...
    #[smb_vector(
        order = 2,
        count(inner(start = 10, num_type = "u16")),
        offset(inner(start = 6, num_type = "u32", subtract = 64))
    )]
